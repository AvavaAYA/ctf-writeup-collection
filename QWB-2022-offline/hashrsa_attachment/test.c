...
#define copy_uint32(p, val)                                                    \
  *((unsigned *)p) = __builtin_bswap32((val)) // gcc 内建函数__builtin_bswap32，
...
void sha256(const unsigned char *data, size_t len, unsigned char *out) {
  ...
  for (int idx = 0; idx < chunk_len; idx++) {
    unsigned val = 0;
    for (int i = 0; i < 64;
         i++) { //将块分解为16个32-bit的big-endian的字，记为w[0], …, w[15]
    ...
    for (
        int i = 16; i < 64;
        i++) { //前16个字直接由以上消息的第i个块分解得到，其余的字由如下迭代公式得到：
      unsigned s0 = rightrotate(w[i - 15], 7) ^ rightrotate(w[i - 15], 18) ^
                    (w[i - 15] >> 3);
      unsigned s1 = rightrotate(w[i - 2], 17) ^ rightrotate(w[i - 2], 19) ^
                    (w[i - 2] >> 10);
